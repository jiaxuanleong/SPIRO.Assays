...
var rootgrowthdir; // output of root growth macro under results subdirectory
var ppdir;		// preprocessing subdir under results subdirectory
	...
	if (step == 2) {
		...
		for (checkstep = 2; checkstep <= finalstep; checkstep ++) {
			...
			for (fullarrayindex = 0; fullarrayindex < totalnoOfgroups; fullarrayindex ++) {
				...
				if (checkstep == 2)  // check for seedlingpositions()
